      ...
      <div class="answer-card" v-if="state === 'end'">
        ...
        <ul class="answer-list">
          <li class="item" :class="{success: isSuccess(q)}" v-for="(q, index) in questions">
            ...
            <h3 v-if="q.type === 'single'">答案：{{ q.options[q.answer] }}</h3>
            ...
            <div v-if="q.type === 'single' || q.type === 'multiple' || q.type === 'aq'">
              <div v-if="q.userAnswer || q.userAnswer === 0">
                <div v-if="q.type === 'single'"> 你的回答：{{ q.options[q.userAnswer] }}</div>
    ...
    this.$ajax({
      ...
      for (let index in questions) {
        this.questions.push({
          ...
          options: op(questions[index].question_option),
          answer: fun(questions[index].answer),
          userAnswer: null
        })
      }
      this.questions.pop()
